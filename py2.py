...
def empilharImagens(imgArray,scale,lables=[]):
    ...
    if rowsAvailable:
        ...
        hor = [imageBlank]*linhas
        hor_con = [imageBlank]*linhas
        for x in range(0, linhas):
        ...
        ver = hor
    if len(lables) != 0:
        eachImgWidth= int(ver.shape[1] / colunas)
        eachImgHeight = int(ver.shape[0] / linhas)
        #print(eachImgHeight)
...
def splitBoxes(img):
    ...
    for r in rows:
        ...
        for box in cols:
            boxes.append(box)
            #cv2.imshow("split",box) #mostra a ultima opcao (circulo) do gabarito
    return boxes
